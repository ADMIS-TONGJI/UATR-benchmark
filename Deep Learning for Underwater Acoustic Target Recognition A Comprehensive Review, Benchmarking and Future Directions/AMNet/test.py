...
from stft import collate_fn  
from model import AMNet

# -------------------------- 1. 超参数配置 --------------------------
HPARAMS = {
    # 设备配置
    "device_id": "0",  # 与训练时一致的GPU编号，None则用CPU
    # 数据路径
    "dataset_root": "./shipsear_noise",
    # 模型配置（核心修改：适配AMNet）
    "checkpoint_path": "./checkpoints/final_epoch_model.pth",  # AMNet模型权重路径
    "num_classes":9,  # 类别数
    "model_version": "T",  # AMNet版本：可选"N"/"T"/"S"
    # 推理参数
    "batch_size": 16,  
    "result_dir": "./test_results",  # 评估结果保存目录
    "cm_fig_name": "confusion_matrix"  # 混淆矩阵图片名
}
...
# -------------------------- 4. 初始化AMNet模型并加载权重--------------------------
def init_model(num_classes: int, model_version: str, checkpoint_path: str, device: torch.device) -> AMNet:
    """
    初始化AMNet模型并加载训练好的权重
    """
    # 初始化AMNet模型
...
# -------------------------- 5. 模型推理并收集标签 --------------------------
@torch.no_grad()
def infer(model: AMNet, test_loader: DataLoader, device: torch.device) -> Tuple[np.ndarray, np.ndarray]:
    """
    推理并收集所有样本的真实标签和预测标签
    Returns:
...
def plot_confusion_matrix(cm: np.ndarray, idx_to_class: dict, save_path: str, cmap: str = "Blues"):
    ...
    plt.title(f"混淆矩阵（AMNet-{HPARAMS['model_version']}+STFT）", fontsize=14, pad=20)  
...
def print_results(overall_metrics: dict, cm: np.ndarray, idx_to_class: dict):
    ...
    """
    print("\n" + "="*80)
    print(f"                    测试集评估结果（AMNet-{HPARAMS['model_version']}+STFT）")  # 更新为AMNet+STFT
    print("="*80)
...
def main():
    ...
    model = init_model(
        ...
        model_version=HPARAMS["model_version"],  
