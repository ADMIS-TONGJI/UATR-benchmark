...
from model import AMNet, init_weights  
...
def train_one_epoch(model: nn.Module, loader: DataLoader, criterion: nn.Module,
    ...
    for batch_idx, (features, labels) in enumerate(loader):
        ...
        total_loss += loss.item() * batch_size  # 累计损失
...
def save_checkpoint(save_path: str, model: nn.Module, optimizer: optim.Optimizer,
    ...
    # 处理多GPU情况（若使用DataParallel，需获取原始模型参数）
...
def main():
    ...
    print("\n=== 初始化AMNet模型 ===")
    model = AMNet(
        model_version=HPARAMS["model_version"],  # 指定模型复杂度版本
        num_classes=HPARAMS["num_classes"]  
    )
    # 初始化模型权重
