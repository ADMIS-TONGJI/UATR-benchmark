...
from LOFAR import collate_fn  
from model import LOFARResNet50  
...
HPARAMS = {
    ...
    "checkpoint_path": "./checkpoints/final_epoch_model.pth",  # 训练好的模型权重路径
    "num_classes": 4,  
    "local_model_path": "./resnet50-19c8e357.pth",  # ResNet50预训练权重路径
    # 推理参数
    "batch_size": 16,  
    # 结果保存
...
def load_test_data(dataset_root: str, batch_size: int) -> Tuple[DataLoader, dict, dict]:
    ...
    if len(test_dataset) > 0:
        ...
        sample_mel, _ = collate_fn([(sample_path, 0)])  # 处理单个样本获取特征维度
        feature_shape = sample_mel[0].shape
    else:
    ...
    print(f"\n=== 测试集信息 ===")
    print(f"特征类型: LOFAR谱")
    print(f"LOFAR特征维度: {feature_shape}")
    print(f"测试集总样本数: {len(test_dataset)}")
    print(f"类别映射（名称→标签）: {class_to_idx}")
    
    return test_loader, class_to_idx, idx_to_class

# -------------------------- 4. 初始化LOFARResNet50并加载权重 --------------------------
def init_model(num_classes: int, local_model_path: str, checkpoint_path: str, device: torch.device) -> LOFARResNet50:
    model = LOFARResNet50(
    ...
    print(f"已初始化LOFARResNet50模型，设备: {device}")
    
    # 检查权重文件是否存在
    if not os.path.exists(checkpoint_path):
        raise FileNotFoundError(f"训练权重文件不存在: {checkpoint_path}")
    if not os.path.exists(local_model_path):
        raise FileNotFoundError(f"ResNet50预训练权重文件不存在: {local_model_path}")
...
def infer(model: LOFARResNet50, test_loader: DataLoader, device: torch.device) -> Tuple[np.ndarray, np.ndarray]:
...
def plot_confusion_matrix(cm: np.ndarray, idx_to_class: dict, save_path: str, cmap: str = "Blues"):
    ...
    plt.title(f"LOFARResNet50 测试集混淆矩阵", fontsize=14, pad=20)
    ...
    plt.savefig(save_path, dpi=300, bbox_inches="tight")  # 高分辨率保存（300dpi）
    plt.close()
...
def print_results(overall_metrics: dict, cm: np.ndarray, idx_to_class: dict):
    """打印所有评估结果，格式清晰"""
    print("\n" + "="*90)
    print("                        LOFARResNet50 模型 - 测试集评估结果")
    print("="*90)
...
def main():
    ...
    print("\n" + "="*90)
    print("LOFARResNet50 模型测试流程全部完成！")
