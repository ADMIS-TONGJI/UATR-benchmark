...
from model import RACNN, init_weights
...
HPARAMS = {
    ...
    # 早停机制
    "early_stop_patience": 20,  # 连续n轮验证集准确率无提升则早停
    # 模型参数
    "input_dim": 2000,  # 与mfcc.py中FLATTENED_SIZE一致
...
# -------------------------- 3. 数据加载 --------------------------
# 核心修改：将tuple[DataLoader, DataLoader]改为Tuple[DataLoader, DataLoader]
...
def train_one_epoch(model: nn.Module, loader: DataLoader, criterion: nn.Module,
    ...
    """训练单轮，返回训练集平均损失和准确率"""
    model.train()  # 切换训练模式（启用BN更新、Dropout等）
    total_loss = 0.0
    total_correct = 0
    total_samples = 0
...
def save_checkpoint(save_path: str, model: nn.Module, optimizer: optim.Optimizer,
    ...
    checkpoint = {
        ...
        "best_val_acc": best_val_acc  # 仅最优模型保存此参数
    }
...
def main():
    ...
    save_checkpoint(
        ...
        optimizer=optimizer,
        scheduler=scheduler,
        epoch=epoch if not (no_improve_epochs >= HPARAMS["early_stop_patience"]) else HPARAMS["epochs"],
        best_val_acc=best_val_acc
    )
