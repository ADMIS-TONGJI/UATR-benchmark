...
HPARAMS = {
    ...
    "local_model_path": "./resnet50-19c8e357.pth",  # ResNet50预训练权重本地路径
    # 保存路径
...
def load_data(dataset_root: str, batch_size: int) -> Tuple[DataLoader, DataLoader]:
    ...
    if len(train_dataset) > 0:
        ...
        sample_mel = collate_fn([(sample_path, 0)])[0][0]
        print(f"LOFAR谱特征维度: {sample_mel.shape}")
...
def main():
    ...
    # 2. 初始化LOFARResNet50模型
    print("\n=== 初始化LOFARResNet50模型 ===")
    model = LOFARResNet50(
        num_classes=HPARAMS["num_classes"],  # 匹配数据集类别数
        local_model_path=HPARAMS["local_model_path"]  # 本地预训练权重路径
    )
