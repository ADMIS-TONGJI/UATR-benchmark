...
from signal_normalize import preprocess_acoustic_signal, collate_fn
...
HPARAMS = {
    ...
    # 早停机制
    "early_stop_patience": 20,  # 连续20轮验证集无提升则早停
    # 模型参数（适配UATC_DenseNet）
    "num_classes": 9,  # 数据集实际类别数（需根据train文件夹下子目录数量调整）
    # 保存路径
...
def load_data(dataset_root: str, batch_size: int) -> Tuple[DataLoader, DataLoader]:
    """加载训练集和验证集（基于时域信号分帧预处理）"""
    # 训练集（从train文件夹划分，非验证集部分）
    ...
    # 验证集（从train文件夹划分，验证集部分）
    val_dataset = UnderwaterAudioDataset(
        root_dir=dataset_root,
        dataset_type="train",
        is_validation=True,
        val_split_ratio=0.15,
        random_seed=42  # 需与训练集种子一致，确保无数据重叠
    )
    ...
    if len(train_dataset) > 0:
        ...
        sample_signal = preprocess_acoustic_signal(sample_path)
        print(f"预处理后信号维度: {sample_signal.shape}")  # 格式：(num_frames, 1, frame_size)
...
def train_one_epoch(model: nn.Module, loader: DataLoader, criterion: nn.Module,
    ...
    for batch_idx, (features, labels) in enumerate(loader):
        ...
        # 前向传播（适配UATC_DenseNet输出：logits为分类得分，probabilities为概率）
        logits, _ = model(features)
        loss = criterion(logits, labels)  # 交叉熵损失计算（基于logits）
...
def save_checkpoint(save_path: str, model: nn.Module, optimizer: optim.Optimizer,
    ...
    checkpoint = {
        ...
        "best_val_acc": best_val_acc,  # 最优验证集准确率
...
def main():
    ...
    optimizer = optim.Adam(
        model.parameters(),
        lr=HPARAMS["init_lr"],
        weight_decay=1e-5  # 轻微L2正则化，防止过拟合
    )
    # 学习率调度器：StepLR（按固定轮次衰减）
    scheduler = StepLR(
        optimizer=optimizer,
    ...
    # 4. 早停机制变量
    best_val_acc = 0.0  # 历史最高验证准确率（用于保存最优模型）
    prev_val_acc = 0.0  # 上一轮验证准确率（用于判断是否“无提升”）
