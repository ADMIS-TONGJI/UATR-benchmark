...
We provide dataset loading routines, feature extraction pipelines, and model architecture implementations for methods in their respective directories. In our implementation, model.py is the core file that implements the main model architecture, dataset.py returns the tuple (WAV audio file path, label) to avoid input data conflicts across different algorithms. Consequently, an additional custom collate_fn is used, which introduces extra I/O overhead. Users may customize the data loader return values as needed—for instance, directly returning (audio feature tensor, label). Considering the personalized requirements (such as multi-GPU DDP/DP training, checkpoint resumption, etc.) and reproducibility, users shall prepare their training scripts following the logical configurations specified below:
